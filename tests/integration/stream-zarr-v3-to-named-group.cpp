...
setup()
{
    ...
    *dim = DIM("x",
    ...
    // configure a new group with the same properties
    ...
    CHECK(ZarrStream_configure_group(stream, &group_props) ==
...
void
verify_group_metadata(const nlohmann::json& meta, bool verify_ome)
{
...
void
verify_file_data(const fs::path& data_root)
{
...
verify()
{
    ...
    CHECK(fs::is_directory(fs::path(test_path) / "A" / "1"));

    {
        ...
        verify_group_metadata(group_metadata, true);
    }

    {
        fs::path group_metadata_path =
          fs::path(test_path) / "A" / "zarr.json";
        EXPECT(fs::is_regular_file(group_metadata_path),
               "Expected file '",
               group_metadata_path,
               "' to exist");
        ...
        verify_group_metadata(group_metadata, false);
    }

    {
        fs::path group_metadata_path =
          fs::path(test_path) / "A" / "1" / "zarr.json";
        EXPECT(fs::is_regular_file(group_metadata_path),
               "Expected file '",
               group_metadata_path,
               "' to exist");
...

    {
        fs::path array_metadata_path =
          fs::path(test_path) / "A" / "1" / "0" / "zarr.json";
        EXPECT(fs::is_regular_file(array_metadata_path),
               "Expected file '",
               array_metadata_path,
               "' to exist");
    ...
    verify_file_data(fs::path(test_path) / "0");
    verify_file_data(fs::path(test_path) / "A" / "1" / "0");
}
...
main()
{
    ...
    try {
        ...
        for (auto i = 0; i < frames_to_acquire; ++i) {
            auto status = ZarrStream_append_to_node(
              stream, "A/1", frame.data(), bytes_of_frame, &bytes_out);
            EXPECT(status == ZarrStatusCode_Success,
                   "Failed to append frame ",
                   i,
                   " to node A/1: ",
