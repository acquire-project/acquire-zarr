...
configure_new_array()
{
    ...
    ZarrArrayProperties properties = {
        .store_key = "new_array",
    ...
    try {
        CHECK(stream->configure_array(&properties) == ZarrStatusCode_Success);
        finalize_stream(stream);

        fs::path new_array_metadata_path =
          fs::path(store_path) / "new_array" / "zarr.json";
        EXPECT(!fs::is_regular_file(new_array_metadata_path),
               "Array metadata file ",
               new_array_metadata_path.string(),
               " should not exist");
...
configure_nested_array()
{
    ...
    try {
        ...
        std::ifstream f(intermediate_group_metadata_path);
...
main()
{
    ...
    try {
        CHECK(configure_new_array());
        CHECK(configure_nested_array());
